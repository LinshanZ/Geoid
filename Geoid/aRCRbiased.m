function result = aRCRbiased(L,upper,psi0)
    load([fileparts(mfilename('fullpath')),'\data\cn2016'],'cn2016');
    cn=cn2016;
    load([fileparts(mfilename('fullpath')),'\data\dcn2016'],'dcn2016');
    dcn=dcn2016;
    delta=eye(L);
    Ekr=E(upper,psi0);
    Dn=zeros(upper,1);
    ...
    for i=2:L
        for j=2:L
                ...
                sum0=sum(EEC(:));
                a(i,j)=(sigma(j)+dcn(j))*delta(i,j)-Ekr(i,j)*sigma(i)-Ekr(j,i)*sigma(j)+sum0;
        end
    end
    result=a(2:L,2:L);
end
